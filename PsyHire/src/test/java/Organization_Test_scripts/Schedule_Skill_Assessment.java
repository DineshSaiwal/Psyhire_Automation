...
import Oragnization_Scenarios.Schedule_Skill_Assessment_Page;

public class Schedule_Skill_Assessment extends Base_Class{
	
	ExtentTest test;
	
	@Test 
	void Schedule_Skill_Assessment_Validation() throws Exception{
	
		test = extent.createTest("Schedule/Re-schedule Skill Assessment Validation").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Organization");
		...
		Schedule_Skill_Assessment_Page page= new Schedule_Skill_Assessment_Page(driver);	
		...
		Thread.sleep(1000);
		 ...
		 Thread.sleep(1000);
		 try {page.Select_Skill_Assessment();
		 ...
		 try {WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10)); 
			 ...
			 if (Actual_Message.contains("Skill Assessment is scheduled")) {
				 test.pass(MarkupHelper.createLabel("Skill Assessment Scheduled Successfully", ExtentColor.GREEN));
				 test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); }
			 else { throw new Exception();}
			 } 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Failed to Schedule Skill Assessment",  ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			test.log(Status.FAIL, "Exception occurred ==> " + e.getMessage());
		}
		 ...
		 try {WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10)); 
      			WebElement element = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@role='alert']")));
      			String Actual_Message = element.getText();
 			 if (Actual_Message.contains("Skill Assessment is re-scheduled")) {
				 test.pass(MarkupHelper.createLabel("Skill Assessment is Re-scheduled Successfully", ExtentColor.GREEN));
				 test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); }
			 else { throw new Exception();} } 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Failed to Re-schedule Skill Assessment", ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			test.log(Status.FAIL, "Exception occurred ==> " + e.getMessage());
		}
